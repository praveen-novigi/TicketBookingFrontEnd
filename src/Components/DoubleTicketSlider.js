...
import DoubleTicketListNew from './DoubleTicketListNew';
...
function Arrow(props) {
 ...
 function TicketSlider() {
    ...
    const settings = {
        ...
        appendDots: dots => (
            <div>
              <button key={dots} className="viewAllFlights" onClick={()=>history.push("/returntickets")}>View All</button>
            </div>
          ),
          customPaging: i => (
            i+1
    ...
    return (
        ...
        {list.map((li,i)=>{
                return (
                    ...
                    <DoubleTicketListNew className="item" list={li}/>
                    </div>
                )
            })}
        </Slider>
